...
import com.crear.auth.repository.RefreshTokenRepository;
...
public class AuthController {
    ...
    @PostMapping("/register-hec")
    public ResponseEntity<RegisterResponse> registerHec(@Valid @RequestBody HecRegisterRequest request) {
        RegisterResponse response = authService.registerHec(request);
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }
    ...
    public ResponseEntity<TokenResponse> login(@Valid @RequestBody LoginRequest request, HttpServletResponse response) {
        ...
        return ResponseEntity.ok()
                ...
                .body(TokenResponse.bearerWithUser(accessToken, refreshToken, 900, userDto));
    }
